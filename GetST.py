...
def getST(defURL:str, charaID:str, charaFileID:str):
    ...
    for _ in range(retry_count):
        if isSaved00 and isSaved01:
            logger.info(charaID + " all normal high pixel stand has saved")
            break
        try:
    ...
    for _ in range(retry_count):
        if isSaved00:
            logger.info(charaID + " all evolution stand has saved")
            break
        try:
            ...
            if data00.status_code == 200:
                logger.info(charaID + " evolution stand has find")
    ...
    for _ in range(retry_count):
        if isSaved00 and isSaved01:
            logger.info(charaID + " all evolution high pixel stand has saved")
            break
        try:
            ...
            if data00.status_code == 200 and not isSaved00:
                logger.info(charaID + " evolution high pixel stand 1 has find")
            ...
            if  data01.status_code == 200 and not isSaved01:
                logger.info(charaID + " evolution high pixel stand 2 has find")
